...
export type InitiationDatum = ConStr0<
  [PubKeyAddress, Value, PubKeyAddress, Integer, BuiltinByteString, Integer]
>;
...
export const initiateEscrowDatum = (
  ...
  const { pubKeyHash: recipient, stakeCredentialHash: recipientStakeCredentialHash } =
    deserializeAddress(recipientAddr);
  const now = Date.now();
...
export const activeEscrowDatum = (
  ...
  const { pubKeyHash, stakeCredentialHash } = deserializeAddress(walletAddress);
  const [initiator, initiatorAmount, _recipient, deadline, contract_ipfs_url, created_at] =
    initiationDatum.fields;
...
export const recipientDepositRedeemer = (recipientAddr: string, depositAmount: Asset[]) => {
...
export const raiseInitiatorDisputeRedeemer = (
...
export const submitRecipientEvidenceRedeemer = (
