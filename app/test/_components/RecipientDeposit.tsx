...
import { Asset, DEFAULT_REDEEMER_BUDGET, deserializeDatum, mergeAssets, MeshValue } from '@meshsdk/core'
import React, { useState } from 'react'
import { activeEscrowDatum, getErrMsg, InitiationDatum, recipientDepositRedeemer, stringifyPlutusData } from '../utils'
...
const RecipientDeposit = () => {
    ...
    const handleRecipientDeposit = async () => {
        ...
        try {
            setIsPending(true)
            const depositAmount: Asset[] = [{ unit: "lovelace", quantity: adaToLovelace(depositAda).toString() }];
    ...
    return (
        <section className="flex flex-col gap-4 border p-4 rounded-md">
            <div>
                <Label className="text-sm font-medium mb-1">Recipient Deposit(ADA)</Label>
                <Input
                    type="number"
                    value={amount}
            ...
            <div>
                <Label className="text-sm font-medium mb-1">Funder TxHash (Initiation UTxO)</Label>
                <Input
                    value={funderTxHash}
