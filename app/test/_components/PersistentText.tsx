...
const PersistentText = ({ data, isJson = false, storageKey }: PersistentTextProps) => {
    ...
    if (isJson) {
        return (
            <div className='relative p-4 bg-neutral-50 border rounded-lg shadow-sm'>
                <pre className='text-xs whitespace-pre-wrap overflow-x-auto max-h-96'>{displayData}</pre>
                <Button
                    size="icon"
                    variant="ghost"
                ...
                >
                    {isCopied ? <CopyCheck className='h-4 w-4 text-green-600' /> : <CopyIcon className='h-4 w-4' />}
                </Button>
            </div>
        )
    }
    ...
    return (
        <div className='flex justify-between items-center bg-neutral-100 px-3 py-2 rounded-lg border'>
            <p className='text-sm font-bold line-clamp-1 truncate' title={displayData}>
                {displayData}
            </p>
            <Button 
                size="icon" 
